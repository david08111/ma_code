...
class Loss_Wrapper():
    ...
    def parse_loss_config(self, loss_type, loss_config):
        ...
        # hierarchical_cluster_batch_contrast - Loss from Paper "Semantic Instance Segmentation with a Discriminative Loss Function" for semantic classes
        ...
        # inter cls loss + intra cls loss (mean
        elif loss_type == "hierarchical_cluster_batch_contrast":
            return Hierarchical_cluster_batch_contrast_loss(**loss_config)

        # hierarchical_cluster_mean_contrast
        # calculation of hinged mse of L2/L1 of each embedding with its epoch globally stored mean cls embedding -> hinged mse of L2/L1 of all mean emb (from curr batch) with globally stored mean emb
        ...
        # TO DO: hierarchical_cluster_all_embeds_contrast
        # calculation of batch mean -> hinged mse of L2/L1 of each embedding with its mean -> hinged mse of L2/L1 of all mean emb combinations
        ...
        # calculation of batch mean -> hinged mse of L2/L1 of each embedding with its mean -> hinged mse of L2/L1 of all mean emb combinations
        # intra cls (all embeds with mean) + inter cls (all means) + (for thing classes intra inst (all inst with inst mean) + inter inst (all inst means))
        ...
        # TO DO: hierarchical_cluster_mean_contrast_panoptic
        elif loss_type == "hierarchical_cluster_mean_contrast_panoptic":
            return Hierarchical_cluster_mean_contrast_panoptic_loss(**loss_config)

        # TO DO: hierarchical_cluster_mean_mov_avg_update_contrast_panoptic
        elif loss_type == "hierarchical_cluster_mean_mov_avg_update_contrast_panoptic":
            return Hierarchical_cluster_mean_mov_avg_update_contrast_panoptic_loss(**loss_config)

        # spherical_contrast_panoptic:
        # sphere radius for class association + cosine similarity for instance discrimination
        ...
        # movable sphere means for class association (contrastive loss with only means as keys/values) + cosine similarity for instance discrimination
        ...
        # movable sphere means for class association (contrastive loss with other embeddings in img as keys/values select also "all" for num_pos_embeds) + cosine similarity for instance discrimination
...
class Hierarchical_cluster_mean_contrast_loss(nn.Module):
    def __init__(self, inter_cls_contrastive_loss, intra_cls_contrastive_loss, reg_loss_norm=2,
                 inter_cls_contrastive_loss_weight=1, intra_cls_contrastive_loss_weight=1, regularization_weight=1):
        super().__init__()
        self.inter_cls_contrastive_loss = Loss_Wrapper(inter_cls_contrastive_loss).loss
        self.intra_cls_contrastive_loss = Loss_Wrapper(intra_cls_contrastive_loss).loss
        # self.num_pos_embeddings_inter = num_pos_embeddings_inter
    ...
    def forward(self, outputs, masks, annotations_data, *args, **kwargs):
        ...
        if len(self.inter_cls_ct_loss_item_class_dict) != num_categories + 1 or len(
                self.intra_cls_ct_loss_item_class_dict) != num_categories + 1 or len(
                self.regularization_loss_item_class_dict) != num_categories + 1:
            for cat_id in embedding_handler.embedding_storage.cls_mean_embeddings.keys():
                # self.abs_radius_err_class_dict[cat_id] = np.zeros(self.bin_elems)
                self.inter_cls_ct_loss_item_class_dict[cat_id] = 0
                self.intra_cls_ct_loss_item_class_dict[cat_id] = 0
        ...
        if self.intra_cls_contrastive_loss_weight > float_precision_thr:
            ...
            for unique_cat_id in unique_cat_ids[1:]:  # skip 0
                ...
                cat_id_mean_embedding_single_curr_batch = torch.mean(outputs_cat_id_embeddings, dim=1)
                cat_id_mean_embedding_single = embedding_handler.embedding_storage.cls_mean_embeddings[unique_cat_id]

                mean_embedding_dict[unique_cat_id] = cat_id_mean_embedding_single_curr_batch
        ...
        if self.inter_cls_contrastive_loss_weight > float_precision_thr:
            for cat_id in mean_embedding_dict:
                ...
                for neg_cat_id in mean_embedding_dict:
                    ...
                    inter_cls_contrastive_loss += self.inter_cls_contrastive_loss(embedding_handler.embedding_storage.cls_mean_embeddings[cat_id],
                                                                                  mean_embedding_dict[neg_cat_id])
    ...
    def log(self, logger, name, epoch, *args, **kwargs):
        ...
        for id in self.inter_cls_ct_loss_item_class_dict:
            if id == "all":
                ...
                logger.add_text(f"{caption} - {self.inter_cls_ct_loss_item_class_dict[id]}", logging.INFO, epoch)
                caption_list = name + ["Item - Inter Class Contrastive Loss",
                                       f"Part - {categories_dict[id]['name']}"]
                logger.add_scalar(caption_list, self.inter_cls_ct_loss_item_class_dict[id], epoch)
...
class Hierarchical_cluster_mean_mov_avg_update_contrast_loss(nn.Module):
    def __init__(self, inter_cls_contrastive_loss, intra_cls_contrastive_loss, reg_loss_norm=2, mov_avg_factor=0.1,
                 inter_cls_contrastive_loss_weight=1, intra_cls_contrastive_loss_weight=1, regularization_weight=1):
        super().__init__()
        self.inter_cls_contrastive_loss = Loss_Wrapper(inter_cls_contrastive_loss).loss
        self.intra_cls_contrastive_loss = Loss_Wrapper(intra_cls_contrastive_loss).loss
        ...
        self.mov_avg_factor = mov_avg_factor
    ...
    def forward(self, outputs, masks, annotations_data, *args, **kwargs):
        ...
        if self.intra_cls_contrastive_loss_weight > float_precision_thr:
            ...
            for unique_cat_id in unique_cat_ids[1:]:  # skip 0
                ...
                cat_id_mean_embedding_single_curr_batch = torch.mean(outputs_cat_id_embeddings, dim=1)
                cat_id_mean_embedding_single = torch.mul(embedding_handler.embedding_storage.cls_mean_embeddings[unique_cat_id], (1-self.mov_avg_factor))

                cat_id_mean_embedding_single_update = torch.add(cat_id_mean_embedding_single, cat_id_mean_embedding_single_curr_batch, alpha=self.mov_avg_factor)

                mean_embedding_dict[unique_cat_id] = cat_id_mean_embedding_single_update

                cat_id_mean_embedding = cat_id_mean_embedding_single_update.repeat(outputs_cat_id_embeddings.shape[1], 1).T
                # cat_id_mean_embedding_tensor = cls_mean_embeddings[unique_cat_id].repeat(1, outputs_cat_id_embeddings.shape[1])
        ...
        if self.inter_cls_contrastive_loss_weight > float_precision_thr:
            for cat_id in mean_embedding_dict:
                ...
                for neg_cat_id in mean_embedding_dict:
                    ...
                    inter_cls_contrastive_loss += self.inter_cls_contrastive_loss(mean_embedding_dict[cat_id],
                                                                                  mean_embedding_dict[neg_cat_id])

            inter_cls_ct_loss_counter = len(mean_embedding_dict) * (len(mean_embedding_dict) - 1) * outputs.shape[0]

        for cat_id in mean_embedding_dict.keys():
            embedding_handler.embedding_storage.cls_mean_embeddings[cat_id] = mean_embedding_dict[cat_id].detach()
...
class Hierarchical_cluster_mean_contrast_panoptic_loss(nn.Module):
    def __init__(self, inter_cls_contrastive_loss, intra_cls_contrastive_loss, inter_inst_contrastive_loss, intra_inst_contrastive_loss, reg_loss_norm=2, inter_cls_contrastive_loss_weight=1, intra_cls_contrastive_loss_weight=1, inter_inst_contrastive_loss_weight=1, intra_inst_contrastive_loss_weight=1, regularization_weight=1):
        super().__init__()
        self.inter_cls_contrastive_loss = Loss_Wrapper(inter_cls_contrastive_loss).loss
        self.intra_cls_contrastive_loss = Loss_Wrapper(intra_cls_contrastive_loss).loss
        ...
        if self.intra_cls_contrastive_loss_weight > float_precision_thr:
            ...
            for unique_cat_id in unique_cat_ids[1:]:  # skip 0
                ...
                if not embedding_handler.dataset_categories[unique_cat_id]["isthing"]:
                    ...
                    cat_id_mean_embedding_single_curr_batch = torch.mean(outputs_cat_id_embeddings, dim=1)
                    cat_id_mean_embedding_single = embedding_handler.embedding_storage.cls_mean_embeddings[
                        unique_cat_id]

                    cls_mean_embedding_dict[unique_cat_id] = cat_id_mean_embedding_single_curr_batch
        ...
        if self.inter_cls_contrastive_loss_weight > float_precision_thr:
            for cat_id in cls_mean_embedding_dict:
                ...
                for neg_cat_id in cls_mean_embedding_dict:
                    ...
                    inter_cls_contrastive_loss += self.inter_cls_contrastive_loss(embedding_handler.embedding_storage.cls_mean_embeddings[cat_id], cls_mean_embedding_dict[neg_cat_id])
...
class Hierarchical_cluster_mean_mov_avg_update_contrast_panoptic_loss(nn.Module):
    def __init__(self, inter_cls_contrastive_loss, intra_cls_contrastive_loss, inter_inst_contrastive_loss,
                 intra_inst_contrastive_loss, reg_loss_norm=2, mov_avg_factor=0.1, inter_cls_contrastive_loss_weight=1,
                 intra_cls_contrastive_loss_weight=1, inter_inst_contrastive_loss_weight=1,
                 intra_inst_contrastive_loss_weight=1, regularization_weight=1):
        super().__init__()
        self.inter_cls_contrastive_loss = Loss_Wrapper(inter_cls_contrastive_loss).loss
        self.intra_cls_contrastive_loss = Loss_Wrapper(intra_cls_contrastive_loss).loss
        self.inter_inst_contrastive_loss = Loss_Wrapper(inter_inst_contrastive_loss).loss
        ...
        if self.intra_cls_contrastive_loss_weight > float_precision_thr:
            ...
            for unique_cat_id in unique_cat_ids[1:]:  # skip 0
                ...
                if not embedding_handler.dataset_categories[unique_cat_id]["isthing"]:
                    ...
                    cat_id_mean_embedding_single_curr_batch = torch.mean(outputs_cat_id_embeddings, dim=1)
                    cat_id_mean_embedding_single = torch.mul(
                        embedding_handler.embedding_storage.cls_mean_embeddings[unique_cat_id],
                        (1 - self.mov_avg_factor))

                    cat_id_mean_embedding_single_update = torch.add(cat_id_mean_embedding_single, cat_id_mean_embedding_single_curr_batch, alpha=self.mov_avg_factor)

                    cls_mean_embedding_dict[unique_cat_id] = cat_id_mean_embedding_single_update

                    cat_id_mean_embedding = cat_id_mean_embedding_single_update.repeat(outputs_cat_id_embeddings.shape[1],
                                                                                1).T
                    # cat_id_mean_embedding_tensor = cls_mean_embeddings[unique_cat_id].repeat(1, outputs_cat_id_embeddings.shape[1])
                    ...
                    for b in range(outputs.shape[0]):
                        ...
                        for unique_segment_id in segment_id_embeddings_dict:
                            ...
                            for neg_segment_id in segment_id_embeddings_dict:
                                ...
                                inter_inst_contrastive_loss += self.inter_inst_contrastive_loss(
                                    segment_id_embeddings_dict[unique_segment_id],
                                    segment_id_embeddings_dict[neg_segment_id])

                                inter_inst_ct_loss_counter += len(segment_id_embeddings_dict) * (
                                            len(segment_id_embeddings_dict) - 1)
                    ...
                    cat_id_mean_embedding_single_update = torch.add(cat_id_mean_embedding_single_curr_batch,
                                                                    cat_id_mean_embedding_single, alpha=self.mov_avg_factor)
        ...
        if self.inter_cls_contrastive_loss_weight > float_precision_thr:
            for cat_id in cls_mean_embedding_dict:
                ...
                for neg_cat_id in cls_mean_embedding_dict:
                    ...
                    inter_cls_contrastive_loss += self.inter_cls_contrastive_loss(
                        embedding_handler.embedding_storage.cls_mean_embeddings[cat_id],
                        cls_mean_embedding_dict[neg_cat_id])

            inter_cls_ct_loss_counter = len(cls_mean_embedding_dict) * (len(cls_mean_embedding_dict) - 1) * \
                                        outputs.shape[0]
