...
if __name__ == "__main__":
    ...
    test2 = torch.optim.__dict__
    test = torch.nn.__dict__
    #
    # test = builder(list(config_dict["architecture"]["decoder"][0].keys())[0], [], **list(config_dict["architecture"]["decoder"][0].keys())[0])
    #
    # pass

    layers = []
    for module in config_dict["architecture"]["modules"]:
        assert len(module) == 1
        name, kwargs = list(module.items())[0]
        if kwargs is None:
            kwargs = {}
        args = kwargs.pop("args", [])
        layers.append(builder(name, *args, **kwargs))
    test = torch.nn.Sequential(*layers)
